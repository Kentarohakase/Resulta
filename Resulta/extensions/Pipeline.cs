namespace Resulta.Extensions
{
  ...
  public class AsyncPipeline<T>
  {
    ...
    /// <summary>
    /// Chains an async step that returns a Result, keeping the same type T.
    /// Skipped if already failed.
    /// </summary>
    public AsyncPipeline<T> ThenAsync(Func<T, Task<Result<T>>> step)
    {
      var next = _task.Bind(v => step(v));
      return new AsyncPipeline<T>(next);
    }

    /// <summary>
    /// Chains an async step that returns a Result with a different type TOut.
    /// Skipped if already failed.
    /// </summary>
    public AsyncPipeline<TOut> ThenAsync<TOut>(Func<T, Task<Result<TOut>>> step)
    {
